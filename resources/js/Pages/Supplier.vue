...
const breadcrumbs = computed(() => [
  ...
  { name: t('breadcrumbs.supplier'), url: '/supplier' },
]);
...
function openModal(supplier = null) {
  if (supplier) {
    ...
    formData.sup_name = supplier.sup_name;
    formData.contact_person = supplier.contact_person;
    formData.mobile = supplier.mobile;
...
// API handling
function submitForm() {
  const url = formData.id ? `/suppliers/${formData.id}` : '/suppliers';
  ...
  router.get('/suppliers', {}, {
    onSuccess: (page) => {
      ...
      gridInstance.updateConfig({
        data: suppliersRef.value.map((supplier) => [
          supplier.id,
          supplier.sup_name,
...
function getColumns() {
  return [
    ...
    t('actions.address'),
    {
      ...
      formatter: (_, row) => {
        try {
          const supplier = suppliersRef.value.find(sup => sup.id === row.cells[0].data);
...
<template>
  <Head :title="$t('pages.authenticate.supplier')" />
  <AuthenticatedLayout>
          ...
          <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
              <div class="card-header d-flex align-items-center justify-content-between">
                <h5 class="mb-0 heading">{{ $t('pages.authenticate.supplier') }}</h5>
                <button
                  type="button"
      ...
      <div class="modal" @click.stop>
        ...
        <form @submit.prevent="submitForm">
          <div class="form-group">
            <label for="sup_name">{{ t('actions.supplier_name') }}:</label>
            <input type="text" id="sup_name" v-model="formData.sup_name" required />
          </div>
          <div class="form-group">
            <label for="contact_person">{{ t('actions.contact_person') }}:</label>
            <input type="text" id="contact_person" v-model="formData.contact_person" required />
          </div>
          <div class="form-group">
